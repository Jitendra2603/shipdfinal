...
class TestDockerExecution(unittest.TestCase):
    ...
    def test_submit_code_success(self):
        ...
        response = requests.post(f"{self.BASE_URL}/submit_code", json={"code": code})
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertTrue(data["code_output"]["success"])
        self.assertTrue(data["db_success"])
        self.assertEqual(data["code_output"]["output"].strip(), "Hello, Docker!")

    def test_submit_code_error(self):
        
        code = "raise ValueError('Test Error')"
        response = requests.post(f"{self.BASE_URL}/submit_code", json={"code": code})
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertFalse(data["code_output"]["success"])
        self.assertTrue(data["db_success"])
        self.assertIn("Test Error", data["code_output"]["error"])
